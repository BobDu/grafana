...
> In Grafana < v10.0, only set once on first-run.
> In Grafana v10.0+, behavior depends on `reset_admin_every_time_run`. When `reset_admin_every_time_run`=`false`, only set once on first-run.
